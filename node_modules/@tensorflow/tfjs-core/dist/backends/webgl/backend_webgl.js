...
var MathBackendWebGL = /** @class */ (function () {
    ...
    MathBackendWebGL.prototype.uploadToGPU = function (dataId) {
        ...
        if (values != null) {
            ...
            if (!isScalar) {
                shapeAs3D = [
                    webgl_util.getBatchDim(shape)
                ].concat(webgl_util.getRowsCols(shape));
            }
            var program = void 0;
            var width = texShape[1], height = texShape[0];
            if (isPacked) {
                _a = tex_util.getPackedMatrixTextureShapeWidthHeight(texShape[0], texShape[1]), width = _a[0], height = _a[1];
                program = new encode_matrix_packed_gpu_1.EncodeMatrixPackedProgram(shapeAs3D, [height, width]);
            }
            else {
                program = new encode_matrix_gpu_1.EncodeMatrixProgram(shapeAs3D, [height, width]);
            }
            var tempDenseInputHandle = this.makeTensorHandle([height, width], dtype);
            this.texData.get(tempDenseInputHandle.dataId).usage = tex_util_1.TextureUsage.UPLOAD;
